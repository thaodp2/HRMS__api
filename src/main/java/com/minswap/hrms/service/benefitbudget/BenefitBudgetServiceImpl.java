...
public class BenefitBudgetServiceImpl implements BenefitBudgetService {
    ...
    public List<BenefitBudgetDto> getBenefitBudgetList(Long managerId, Long personId, Integer page, Integer limit, Long requestTypeId, String search, Integer month, Year year, String sort, String dir) {
        ...
        if (requestTypeId != CommonConstant.REQUEST_TYPE_ID_OF_OT) {
            if (page == null && limit == null) {
                pageInfor = leaveBudgetRepository.getBenefitBudgetListWithoutPaging(year == null ? Year.now() : year, (search == null || search.trim().isEmpty()) ? null : search.trim(), managerId, requestTypeId, personId, dirSort == null ? Sort.by(Sort.Direction.DESC, "rollNumber") : Sort.by(dirSort, sort));
            } else {
                pageInfor = leaveBudgetRepository.getBenefitBudgetList(year == null ? Year.now() : year, (search == null || search.trim().isEmpty()) ? null : search.trim(), managerId, requestTypeId, personId, PageRequest.of(page - 1, limit, dirSort == null ? Sort.by(Sort.Direction.DESC, "rollNumber") : Sort.by(dirSort, sort))).getContent();
            }
        } else {
            if (page == null && limit == null) {
                pageInfor = otBudgetRepository.getBenefitBudgetListWithoutPaging(month == null ? java.time.LocalDateTime.now().getMonthValue() : month, year == null ? Year.now() : year, (search == null || search.trim().isEmpty()) ? null : search.trim(), managerId, personId, dirSort == null ? Sort.by(Sort.Direction.DESC, "rollNumber") : Sort.by(dirSort, sort));
            } else {
                pageInfor = otBudgetRepository.getBenefitBudgetList(month == null ? java.time.LocalDateTime.now().getMonthValue() : month, year == null ? Year.now() : year, (search == null || search.trim().isEmpty()) ? null : search.trim(), managerId, personId, PageRequest.of(page - 1, limit, dirSort == null ? Sort.by(Sort.Direction.DESC, "rollNumber") : Sort.by(dirSort, sort))).getContent();
            }
        }
        return pageInfor;
    }
